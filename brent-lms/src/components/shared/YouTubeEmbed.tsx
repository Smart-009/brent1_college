...
export function YouTubeEmbed({
  ...
  // Reset rotated event on component unmount
  useEffect(() => {
    return () => {
  ...
  const handleToggleRotation = async (e?: React.MouseEvent) => {
    ...
    window.dispatchEvent(new CustomEvent('eclat-video-rotated', { detail: { isRotated: nextRotated } }))
  ...
  const setPlayerScreenSize = async (mode: 'small' | 'medium' | 'full') => {
    ...
    if (mode === 'full') {
      ...
      if (isMobile) {
        ...
        window.dispatchEvent(new CustomEvent('eclat-video-rotated', { detail: { isRotated: true } }))
      }
